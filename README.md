...
Validation — Does the model generalise to a real city it has never seen?
---
Data Sources
Source	Contents	Usage
Zillow Research (2023–2026)	Home prices by U.S. zip code	Calculated 3-year ROI
U.S. Census Bureau	Median household income by zip code	Matched to each Zillow zip code
NCES Common Core of Data	Every public school location in the U.S.	Counted schools per city (school density)
...
Training Pipeline
Split — 80% training (21,111 rows), 20% held-out test (5,278 rows)
Fit — 1,000 trees learn rules from the training set
Aggregate — tree predictions are averaged into a final ROI estimate
Evaluate — predictions are compared against the unseen test set
...
> **The safety floor finding:** School density doesn't push ROI up, but it prevents it from collapsing. Areas with many schools are significantly more resilient during economic downturns.
---
...
---
References
Zillow Research. "Zillow Home Value Index (ZHVI) Time Series," 2024. zillow.com/research/data
U.S. Census Bureau. "ACS 5-Year Estimates: Median Household Income (Table B19013)," 2024. data.census.gov
NCES. "Common Core of Data: Public School Universe," 2024. nces.ed.gov/ccd
